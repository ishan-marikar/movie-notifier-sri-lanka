...
EAPMovieNotifier.prototype.listen = function() {
  ...
  this.iterator = setInterval(function() {
    getMovies(function(error, movies) {
      ...
      that.emit('loop', movies);

      debug('Movies: ' + JSON.stringify(movies));
      ...
      // HACK: Bypassing autocorrect because of stupid reasons.
      var correctedWord = searchQuery;

      movies.forEach(function(movie) {
        if (new RegExp(correctedWord).test(movie)) {
          movie = movie.trim();
          debug('Found ' + movie);
          that.emit('found', movie);
          that.stop();
        }
      });
    });
